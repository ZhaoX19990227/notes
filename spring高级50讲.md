...
1. 什么是 BeanFactory
   ...
   **context.getBean方法底层实际上还是间接调用了BeanFactory**
   ...
   public static void main(String[] args) throws NoSuchFieldException, IllegalAccessException {
       ...
       map.entrySet().stream().filter(e -> e.getKey().startsWith("component"))
           .forEach(e -> {
               ...
               //component1=com.example.controller.Component1@4c9e9fb8
               //component2=com.example.controller.Component2@9ec531
           });
   }
   }
   ```
...
3.  ApplicationContext 比 BeanFactory 多点啥
   ...
   * 国际化（MessageSource接口）、通配符方式获取一组 Resource 资源（ResourcePatternResolver接口）、整合 Environment 环境(ApplicationEventPublisher接口)、事件发布与监听(EnvironmentCapable接口)
   
   
   国际化：context.getMessage(key);  根据key获取翻译后的信息  需要提前配置好文件
   
   Resource 资源：getResources();根据通配符获取资源
   
   获取环境变量：getEnvironment().getProperty("java_home");也可以获取配置文件中的键值对
   
   发布事件：context.publishEvent(事件对象);  收集事件的就是Spring中所有的组件，需要在方法上添加@EventListener
...
public class MyAspect {
    ...
    @Before("execution(* com.zx.aspectJ.MyService.foo())")
    public void before() {
        log.info("before()");
    }
}
...
public class Application {
  private static final Logger log = LoggerFactory.getLogger(MyAspect.class);
  public static void main(String[] args) {
    new MyService().foo();//这里并没有走代理  是因为走的是pom文件中aspectj-maven-plugin的代理 是将编译好的class文件进行增强
...
public class JdkProxyDemo {
    ...
    public static void main(String[] param) {
        ...
        // 代理对象    为什么需要类加载器：代理类是没有源码的  是在类加载的过程中直接生成代理类的字节码，这个字节码文件也是需要加载的  所以需要类加载器。
...
1. DispatcherServlet 是在第一次被访问时执行初始化, 也可以通过配置修改为 Tomcat 启动后就初始化
   ...
   ```properties
   spring.mvc.servlet.load-on-startup = 1   1代表优先级  数字越小优先级越高
   ```
